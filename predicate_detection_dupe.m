...
for ii = 1 : testNum
    ...
    if len ~= 0
        ...
        rlp_confs_ours_dupe{ii} = zeros(len * 70, 1);
        rlp_labels_ours_dupe{ii} = zeros(len * 70, 3);
        sub_bboxes_ours_dupe{ii} = reshape(repmat(reshape(...
            sub_bboxes_ours{ii}, [1, len, 4]), [70, 1, 1]), [len * 70, 4]);
        obj_bboxes_ours_dupe{ii} = reshape(repmat(reshape(...
            obj_bboxes_ours{ii}, [1, len, 4]), [70, 1, 1]), [len * 70, 4]);
    else
        rlp_confs_ours{ii} = []; 
        rlp_confs_ours_dupe{ii} = [];
    ...
    for jj = 1 : len
        ...
        for kk = 1 : 70
            idx = (jj - 1) * 70 + kk;
            rlp_labels_ours_dupe{ii}(idx, 1) = k1;
            rlp_labels_ours_dupe{ii}(idx, 3) = k2;
            rlp_labels_ours_dupe{ii}(idx, 2) = kk;
            rlp_confs_ours_dupe{ii}(idx) = rlpScore(kk);
        end
    end
    cc = 1;
